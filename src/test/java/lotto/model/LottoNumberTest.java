...
class LottoNumberTest {
    ...
    @Test
    void 로또_번호가_0이면_예외가_발생해야_한다() {
        assertThatThrownBy(() -> LottoNumber.from(0))
                .isInstanceOf(IllegalArgumentException.class);
    }

    @Test
    void 로또_번호가_45초과이면_예외가_발생해야_한다() {
        assertThatThrownBy(() -> LottoNumber.from(46))
                .isInstanceOf(IllegalArgumentException.class);
    }

    @Test
    void 로또_번호가_음수이면_예외가_발생해야_한다() {
    ...
    void 보너스_번호가_추첨_번호와_중복되는_값을_가지면_예외가_발생해야_한다() {
        ...
        assertThatThrownBy(() -> LottoNumber.bonusNumber(bonusNumber, winningNumbers.getNumbers()))
                .isInstanceOf(IllegalArgumentException.class);
    }
}
